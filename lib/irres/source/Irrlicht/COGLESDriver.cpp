...
COGLES1Driver::~COGLES1Driver()
{
	...
	if (GL_TRUE != eglMakeCurrent(EglDisplay, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT))
        os::Printer::log("eglMakeCurrent failed");

    bool test = testEGLError();

	if (GL_TRUE != eglDestroyContext(EglDisplay, EglContext))
        os::Printer::log("eglDestroyContext failed");

    test = testEGLError();

	if (GL_TRUE != eglDestroySurface(EglDisplay, EglSurface))
        os::Printer::log("eglDestroySurface failed");

    test = testEGLError();

#ifndef _IRR_COMPILE_WITH_WINDOWS_DEVICE_
 	if (GL_TRUE != eglTerminate(EglDisplay))
        os::Printer::log("eglTerminate failed");
