...
:Data(0), Size(size), Format(format), DeleteMemory(true), ImageDataSizeInBytes(0)
{
	initData();
}
...
: Data(0), Size(size), Format(format), DeleteMemory(deleteForeignMemory), ImageDataSizeInBytes(0)
{
	if (ownForeignMemory)
	{
		Data = (u8*)0xbadf00d;
...
LodData(0),
Size(size),
ImageDataSizeInBytes(imageDataSizeInBytes),
Format(format),
CustomMipmapCount(mipmapCount),
HasCustomMipmaps(false),
DeleteMemory(deleteForeignMemory)
{
    if (ownForeignMemory)
    {
        Data = (u8*)0xBADF00D;
    ...
    //TODO
    /*if(CustomMipmapCount > 0)
    {
        LodData = new void*[CustomMipmapCount + 1];
        u8* dataPtr = reinterpret_cast<u8*>(Data);
        u32 width = Size.Width;
        u32 height = Size.Height;
        u32 i = 0;
        for(; i < CustomMipmapCount; ++i)
        {
            u32 levelSize = 
                pixel_format::computeMipmapSizeInBytes(format,
                width,
                height,
                i,
                false);
        ...
        _IRR_DEBUG_BREAK_IF(dataPtr 
            + pixel_format::computeMipmapSizeInBytes(format,
            width,
            height,
            i,
            false)
            - reinterpret_cast<u8*>(Data) == std::ptrdiff_t(imageDataSizeInBytes));
        LodData[i] = 0;
    }*/
}
...
void CImage::initData()
{
	...
	BytesPerPixel = getBitsPerPixelFromFormat(Format) / 8; //TODO: Old call. Still here for backward comp. Will be removed soon

    if (BytesPerPixel == 0)
    {
        BytesPerPixel = irr::video::getBytesPerPixel(Format);
        Pitch = computePitch(Format, Size.Width);

        if (ImageDataSizeInBytes == 0)
            ImageDataSizeInBytes = Size.Height * Pitch;

        if (!Data)
        {
            _IRR_DEBUG_BREAK(ImageDataSizeInBytes > 0);
            Data = new u8[ImageDataSizeInBytes];
        }
