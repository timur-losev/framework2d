...
namespace irr
{
    namespace video
    {
        ...
        public:
            ...
            STextureDesc()
                : Type(ETT_2D)
                , Format(ECF_A8R8G8B8) // historical reasons to that default format
                , Layout(ETL_LINEAR)
                , Usage(EBU_STATIC)
                , Size(1, 1)
                , Depth(1)
                , WithMipmaps(false)
                , RenderTarget(false)
                , MarkAsUnloadable(false)
            {
            }

            //!
            STextureDesc(E_TEXTURE_TYPE type ,
                ECOLOR_FORMAT colorFormat,
                E_TEXTURE_LAYOUT layout,
                ...
                : Type(type)
                , Format(colorFormat) // historical reasons to that default format
                , Layout(layout)
                , Usage(usage)
                , Size(size)
                , Depth(depth)
            ...
            bool operator == (const STextureDesc& other) const
            {
                return (Type == other.Type
                    ...
                    && Size == other.Size
                    && Depth == other.Depth
                    && WithMipmaps == other.WithMipmaps
                    && RenderTarget == other.RenderTarget
                    && MarkAsUnloadable == other.MarkAsUnloadable);
            }
            ...
            //! The texture can be removed from VRAM when backgrounding an app (Apple iOS4)
            bool MarkAsUnloadable;
        };
