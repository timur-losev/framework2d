...
#if defined(WIN64) || defined(_WIN64) // using portable common solution for x64 configuration
#include <crtdbg.h>
#define _IRR_DEBUG_BREAK( _CONDITION_ ) if (!(_CONDITION_)) {_CrtDbgBreak();}
#else
#define _IRR_DEBUG_BREAK( _CONDITION_ ) if (!(_CONDITION_)) {_asm int 3}
#endif
#else
#include "assert.h"
#define _IRR_DEBUG_BREAK( _CONDITION_ ) assert( !!(_CONDITION_) );
#endif
#else
#define _IRR_DEBUG_BREAK( _CONDITION_ )
#endif
